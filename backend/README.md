...
- **点赞**：服务端只管累加，**「我点过了」由浏览器的 localStorage 记住**
  （前端 `src/data/reactions.ts`，键 `lizao:liked:<slug>`）。
  代价是清了缓存或者换浏览器能再点一次 —— 对个人站来说这个强度是对的：
  赞表达的是「有几个人路过觉得不错」，不是一个要拿去对账的数字。
  真到有人刷的地步，再上 IP 指纹（会误伤同一个 WiFi 下的人）也不迟。
