...
/**
 * 三叠书。**右侧那叠横放的是门（去留言）**，另外两叠只是画。
 *
 * 竖排那叠（books-tall）曾经是通向 /notes 的热点，那个地址随笔记页
 * 一起废除，它降级成了纯装饰（现在是注释掉的那一行 —— 连画都不画了）。
 * 2026-09-23 把门重新接上，但**接在右侧横叠那叠上**（owner 指定）
 * 而不是复活竖排那叠：屋里三叠书里就它没被别的东西压着，
 * 而且它左边就是桌子边缘，点着顺手。
 *
 * 所以这个数组里**混着一扇门**（books-right）—— 它同时是热点，
 * 热点本体写在下面 bookDoor 里。名字从 bookDecor 改成 bookLayers
 * 就是因为这个：“装饰”不再是这三叠书的全部。
 */
const bookLayers: SceneLayer[] = [
...
/**
 * 右侧那叠横放的书：一扇门。位置与**绘制顺序**由 bookLayers 里那一行给，
 * 这里只补去向与可点区域（两边同一个 id，SceneStage 会把它们接起来）。
 *
 * **可点区域就是整幅框**（四个 0），这是量过之后的选择，不是偷懒：
 * 这张图的 alpha 覆盖框内 86%，而且**不透明部分四边都顶到框沿**
 * （`getbbox()` 就是整框）—— 框外的空白早在导出素材时就裁掉了，
 * 所以这框就是这叠书的轮廓本身。
 *
 * 压在它上面的只有那只马克杯，压在它下面的只有桌面 —— 两者都不接鼠标
 * （不是一个热点，也就没有“谁抢谁”的问题）。**代价要说清楚：
 * 点杯子的地方也会去留言。** 重叠那块的实测：杯子（按原 zone 脚本的
 * 松弛量膨胀 14px 后）盖住书本体像素的 11.9%，都在左下角。
 * 接受它，是因为另一条路更差 —— 把杯子挖出去，就得在书的左下角
 * 留一块**点了什么也不发生**的死区，而热点的毛病从来是“占太多”，
 * 不是“占太少”。
 *
 * （原脚本 `scene-build-zones.py` 只为**热点之间**让位：它把上面一层
 * 选中的 zone 矩形标成已占，而垃圾桶级的装饰根本不在它的名单里 ——
 * 所以它跑出来也会是这一整框，只不过要重新跑它得先把仓库里没有的
 * 源 PNG（`frontend/1/`）和 numpy 找回来。）
 */
const bookDoor: SceneHotspot = {
  id: 'books-right',
  label: '点击书本，去留言',
  to: '/guestbook',
  ...
  width: 11.52,
  alt: '桌上横放着的一叠书',
  zones: [{ left: 0, top: 0, right: 0, bottom: 0 }],
}

/**
 * 另外两扇门。标签写的是**真实去向**，不是物件本身的名字：
 * 电脑上写「关于我」但跳去做过的项目，就是在骗人。
 * （第三扇门是一叠书，它在上面 —— 位置要从绘制顺序那个数组里拿。）
 */
 ...
 *
 * 书重新变成一扇门之后（books-right），这一条**仍然只管观感** ——
 * 它那一个整框 zone 与电脑、相机都不重叠（见上面 bookDoor 那段）。
 *
 * 位置在这里写一次、两边共用：`layers` 里的 books-right 决定**画在哪、叠在谁前面**，
 * `hotspots` 里的 bookDoor 决定**点了去哪**（同一个 id，SceneStage 自己接）。
 * **别把它只写进 hotspots**：那份列表里没登记的层会被当成“只有热点”追在最后画，
 * 书会跑到杯子前面去。
 *
...
export const roomScene: Scene = {
  ...
  layers: [...roomLayers, ...bookLayers, ...roomDoors, mugLayer],
  /** 三扇门：书（留言）、电脑（做过的）、相机（拍下的）。 */
  hotspots: [...roomDoors, bookDoor],
}
 ...
 *   - 下：最靠上的一件家具是右侧那叠书（books-right，top 69.35%，
 *     它的左上角就在文字块的右半边），再往下是笔记本（top 64.67%）。
 * 所以文字块底边不能超过 y 60%。
 * （竖排那叠书曾经也在这个边界里 —— 它 2026-09-23 起连画都不画了，
 *   这里留着它当时的数只为说明“那一整段墙都是干净的”。）
 *
